...
function bloc(t, d) {
  ...
  h += '<textarea class="jot" data-note="' + t.id + ":" + d.n + '" placeholder="Notes du jour ' + d.n +
...
function vBudget() {
  ...
  h += '<div class="form"><input id="fq" type="text" placeholder="Quoi ?" style="flex:1;min-width:150px">' +
    '<input id="fm" type="number" min="0" step="1" placeholder="Montant">' +
    '<select id="fd"><option value="yen">¥</option><option value="eur">€</option></select>' +
    '<select id="fp">' + t.budget.map(function (b) { return '<option>' + b[0] + '</option>'; }).join("") + '</select>' +
    '<select id="fe"><option value="">— étape —</option>' +
...
function vCarnet() {
  var h = '<div class="pad"><section><p class="eyebrow">Sur place</p><h2>Le <em>carnet</em></h2>' +
    ...
    '<textarea class="jot" id="carnet" style="min-height:340px;font-size:15px" ' +
...
function nav() {
  ...
  h += '<div class="pick"><label>Itinéraire</label><select id="pick">' +
...
function sauvegarde() {
  ...
  d.innerHTML = '<div class="box"><h3>Sauvegarde</h3>' +
    ...
    '<textarea class="jot" id="mj" spellcheck="false" style="min-height:190px;font-family:var(--f-num);font-size:11.5px"></textarea>' +
  ...
  d.querySelector("#mdl").onclick = function () {
    try {
      var blob = new Blob([d.querySelector("#mj").value], { type: "application/json" });
      var u = URL.createObjectURL(blob), a = document.createElement("a");
      a.href = u; a.download = "carnet-japon.json";
      document.body.appendChild(a); a.click(); a.remove();
      setTimeout(function () { URL.revokeObjectURL(u); }, 1000);
      dit("Si rien ne s'est téléchargé, copiez le texte ci-dessus : certaines pages publiées bloquent les téléchargements.");
    } catch (e) { dit("Téléchargement impossible ici — copiez le texte ci-dessus."); }
  };
